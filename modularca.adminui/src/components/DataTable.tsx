...
export function DataTable<Row>({
    ...
    // The "filler" column(s) soak up leftover horizontal space and stay flush to the right edge.
    // Explicit `flex` columns win; otherwise the LAST visible column fills by default so empty
    // space always lands on the right (not on a middle/left column). Fillers aren't manually
    // resizable — they take whatever the fixed columns leave behind.
    const flexKeys = useMemo(() => {
        const explicit = visibleCols.filter((c) => c.flex).map((c) => c.key);
        if (explicit.length) return new Set(explicit);
        return new Set(visibleCols.length ? [visibleCols[visibleCols.length - 1].key] : []);
    }, [visibleCols]);
