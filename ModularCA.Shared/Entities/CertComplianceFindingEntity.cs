using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;

namespace ModularCA.Shared.Entities;

/// <summary>
/// Records a compliance finding detected during automated certificate scanning.
/// Each row ties a specific finding type (weak key, deprecated algorithm, etc.) to a
/// certificate and tracks resolution state.
/// </summary>
[Table("CertComplianceFindings")]
public class CertComplianceFindingEntity
{
    /// <summary>Unique identifier for this compliance finding.</summary>
    [Key]
    public Guid Id { get; set; } = Guid.NewGuid();

    /// <summary>Foreign key to the certificate this finding relates to.</summary>
    public Guid CertificateId { get; set; }

    /// <summary>Severity level: Critical, Warning, or Info.</summary>
    [MaxLength(50)]
    public string Severity { get; set; } = "Warning";

    /// <summary>
    /// Finding type identifier such as WeakKey, DeprecatedAlgorithm,
    /// OverLongValidity, MissingSANs, ExpiringWithoutRenewal, or WildcardCert.
    /// </summary>
    [MaxLength(255)]
    public string Type { get; set; } = string.Empty;

    /// <summary>Human-readable description of the finding.</summary>
    [MaxLength(500)]
    public string Description { get; set; } = string.Empty;

    /// <summary>UTC timestamp when the finding was first detected.</summary>
    public DateTime DetectedAt { get; set; } = DateTime.UtcNow;

    /// <summary>Whether this finding has been resolved (manually or automatically).</summary>
    public bool IsResolved { get; set; } = false;

    /// <summary>UTC timestamp when the finding was marked as resolved.</summary>
    public DateTime? ResolvedAt { get; set; }
}
